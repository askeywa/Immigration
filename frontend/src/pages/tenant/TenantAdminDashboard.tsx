...
export const TenantAdminDashboard: React.FC = () => {
  ...
  // Load dashboard data (with duplicate call protection)
  const loadDashboardData = useCallback(async () => {
    console.log('🔍 TenantAdminDashboard: loadDashboardData called');
    
    // Prevent duplicate calls
    if (isCurrentlyLoading.current || hasLoadedData.current) {
      console.log('⚠️ Skipping duplicate API call - already loading or loaded');
      return;
    }
    
    if (!isTenantAdmin) {
      console.log('❌ Not tenant admin, skipping data load');
      return;
    }
    ...
    try {
      const [statsResponse, activityResponse] = await Promise.all([
        tenantApiService.get('/tenant/stats'),
        tenantApiService.get('/tenant/recent-activity')
      ]);

      if (statsResponse.success) {
        setTenantStats(statsResponse.data.data);
      ...
      if (activityResponse.success) {
        ...
        console.log('❌ Activity response failed:', activityResponse);
      }
      ...
      log.error('Failed to load tenant admin dashboard data', { error: err.message });
  ...
  }, [isTenantAdmin]); // Stable dependency

  // Load data on component mount (with proper dependencies)
  useEffect(() => {
    if (isTenantAdmin && !hasLoadedData.current) {
      loadDashboardData();
    }
  }, [isTenantAdmin, loadDashboardData]); // Proper stable dependencies
  
  // Debug rendering state (reduced logging)
  // console.log('🔍 TenantAdminDashboard Render State:', {
  //   isLoading,
  //   hasError: !!error,
  ...
  // console.log('🔍 TenantAdminDashboard Debug:', {
  //   isTenantAdmin,
  //   isSuperAdmin,
  //   isTenantUser,
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50">
      {/* Dashboard Header - Clean and Modern */}
